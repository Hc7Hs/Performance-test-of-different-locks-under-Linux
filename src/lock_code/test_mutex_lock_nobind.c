...
{
    if (signo == SIGINT)
    {	//Print the number of locks per thread;	
		int i;
		for(i=0;i<nr_parallel_threads;i++){
			printf("\nThe thread %d,count:%d\n",i,thread_param[i].count);
		}
        printf("lock_counter=%ld\n", lock_counter);
		clock_gettime(CLOCK_MONOTONIC, &tm_ended);  // Get the monotonic time at the end of the thread	authorï¼hh
...
int main(int argc, char *argv[])
{
	...
	for (i = 0; i < nr_parallel_threads; ++i) {
		...
		lock_counter += thread_param[i].count;
	}
